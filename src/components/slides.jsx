...
import { decryptGuestImage } from '../lib/decrypt-image.js'

/* Guest map: slug → { h: hashedFilename }. Generated by seal-guests.cjs.
   This file always exists because the seal script creates it (even if empty). */
import guestMapRaw from '../data/.guest-map.json'
const guestMap = guestMapRaw || {}
...
function Personal({ guest, slug }) {
  ...
  useEffect(() => {
    ...
    async function loadImage() {
      // Try encrypted image first
      const mapEntry = slug && guestMap[slug]
      if (mapEntry && mapEntry.h) {
        const blobUrl = await decryptGuestImage(slug, mapEntry.h)
      ...
      // Fall back to plaintext image (for local dev without seal)
      // Try slug-based filenames in common extensions
      if (slug) {
        for (const ext of ['.jpg', '.jpeg', '.png', '.webp']) {
          const src = `${import.meta.env.BASE_URL}guests/${slug}${ext}`
          try {
        ...
        <FadeUp delay={0.35} className="flex justify-center py-1">
          <span className="photo-frame">
            <span className="photo-frame__inner">
              {imgSrc ? (
                <img src={imgSrc} alt={guest.name} onError={() => setImgSrc(null)} />
              ) : (
...
export function buildSlides(guest, slug) {
  const slides = []
  if (guest) slides.push({ key: 'personal', node: <Personal guest={guest} slug={slug} /> })
